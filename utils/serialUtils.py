    ...
    def read(self, over_time=0.2):
        ...
        while True:
            ...
            while time.time() - start_time < over_time:
                ...
                data = str(data.decode('utf-8', 'ignore').strip(b'\x00'.decode('utf-8')))
                if data != '':
...
class Engine:
    ...
    def motor_rotate(self, motor, per=Protocol.hundred, timeout=60):
        assert self.sc.state.get_position(motor) == Protocol.reseted
        self.sc.motor_command(motor, Protocol.forward, timeout, per)
            
            
    def motor_rotate_multi(self, motor: tuple, timeout=60):
        rotate_m = []
        rotate_d = []
        rotate_p = []
        for m in motor:
    ...
    def motor_rollback_multi(self, motor: tuple, timeout=60):
        rotate_m = []
        rotate_d = []
        rotate_p = []
        for m in motor:
            rotate_m.append(m)
            rotate_d.append(Protocol.backward)
            rotate_p.append(Protocol.hundred)
        if rotate_m:
            self.sc.motor_command_multi(tuple(rotate_m), tuple(rotate_d), timeout, tuple(rotate_p))
                
                
    def motor_rollback(self, motor, per=Protocol.hundred, timeout=60):
        if motor != Protocol.base_motor:
            assert self.sc.state.get_position(motor) == Protocol.not_reseted
    ...
    def rotate_revised(self):
        ...
        if self.sc.state.params['malfunction'] == True:
            return
        self.motor_rollback(Protocol.base_motor, Protocol.fifty)
        if self.sc.state.params['malfunction'] == True:
            return
    ...
    def whole_process(self):
        ...
        if self.sc.state.params['malfunction'] == True:
            return
        self.motor_rotate_multi((Protocol.forward_pressing_board_motor, Protocol.pressing_board_motor))
        if self.sc.state.params['malfunction'] == True:
            return
        self.deactivate_vacuum_pump()
        if self.sc.state.params['malfunction'] == True:
        ...
        if self.sc.state.params['malfunction'] == True:
            return
        self.activate_vacuum_pump()
        self.motor_rollback_multi((Protocol.forward_pressing_board_motor, Protocol.pressing_board_motor))
        if self.sc.state.params['malfunction'] == True:
            return


    def in_out_demo(self):
        ...
        if self.sc.state.params['malfunction'] == True:
            return
        self.motor_rollback_multi((Protocol.pushing_book_motor, Protocol.lift_motor))
        if self.sc.state.params['malfunction'] == True:
            return
        self.whole_process()
